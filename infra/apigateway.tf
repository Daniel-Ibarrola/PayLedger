...
resource "aws_apigatewayv2_integration" "items_service" {
  api_id                 = aws_apigatewayv2_api.main.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.items_service.invoke_arn
...
resource "aws_apigatewayv2_route" "put_item" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.items_service.id}"
}
...
resource "aws_lambda_permission" "api_gateway" {
  ...
  function_name = aws_lambda_function.items_service.function_name
